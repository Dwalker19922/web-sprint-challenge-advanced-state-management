...
#### reducers/index.js
  ...
  * [d ] Adds the following state values into the initialState:
      - an array of smurfs
      - a boolean indicating if the app is loading
      - a string indicating a possible error message

  * [d ] Add in the arguments needed to complete a standard reducer function.
  * [d ] Add in a reducer case to accomidate the start of a smurf fetch.
  * [d ] Add in a reducer case to accomidate the successful smurf api fetch.
  * [d ] Add in a reducer cases to accomidate the failed smurf api fetch.
  * [ d] Add in a reducer case to accomidate adding a smurf (including the name, nickname, position, summary and an internally generated id) into your smurf list.
  * [d ] Add in a reducer case that adds in a value to the error message.
...
#### actions/index.js
  ...
  * [d ] Add a thunk action called fetchSmurfs that triggers a loading status display in our application, performs an axios call to retreive smurfs from our server, saves the result of that call to our state and shows an error if one is made.
  * d[ ] Add a standard action that allows us to add new smurf (including the name, nickname, position, summary).
  * [d ] Add a standard action that allows us to set the value of the error message slice of state.
  
#### App.js
  Connect this component to redux **passing in your fetchSmurfs action only.** 
  
  * [d ] Connect the fetchSmurfs actions to the App component.
  * [d ] Call the fetchSmurfs action when the component first loads.

#### components/SmurfList.js
  Connect this component to redux, **passing in your smurf array and loading status state values.** 
  
  * [d ] Connect the smurfs and loading state values to the SmurfList component.
  * [ d] Replace the single Smurf component instance with a map return a Smurf component for each entry in the smurfs list.
  * [d ] Replace the static isLoading variable with the state loading variable.
...
#### components/AddForm.js
  ...
  * [d ] Connect the errorMessage, setError and addSmurf actions to the AddForm component.
  * [d ] Replace all instances of the errorMessage static variable with your error message state value. 
  * [ d] Within the handleSubmit function, replace the static assignment to errorMessage with a call to the setError action. Test that an error is displayed when this validation code fails.
  * [ d] Within the handleSubmit function, call your addSmurf action with the smurf name, position, nickname and summury passed as arguments. Test that a smurf is correctly added to when the form is submitted.
...
* [ d] Build a smurf profile page with using react-router and dynamic url params  
* [ d] Using React Testing Library, test one or more of your components (not any component that fetches data though)
...
- [ d] Retrieve an array all the Smurfs in the Smurf DB by writing a `GET` to the endpoint `http://localhost:3333/smurfs`.
- [d ] Double check that your response from the server is an array of smurfs.
...
- [ d] Double check to make sure that a smurf is created correctly once your functionality is built out. Initially Poppa and Smurfette will be in the array.
